...
<head>
	<meta charset="UTF-8">
	<title>공지사항 페이징 목록 NoticeListPage</title>
	<!--  부트스트랩 5.3 적용 -->
	...
	<form class="form-horizontal" id="frm">
		<div class="form-group">
			<div>
				...
				<h4 align="center"><font color="FF5050">총 건수 : ${ncount}&emsp;현재 ${select}페이지</font></h4>
			</div>
		</div>
		<!-- 게시글 쓰기/등록 버튼 -->
		<p align="center">
		 ...
		 <div align="center">
			<c:if test="${prev}">
				 <span>[ <a href="/CS/board/NoticeListPage?num=${startPageNum - 1}">이전</a> ] </span>
			</c:if>
			...
			<c:forEach begin="${startPageNum}" end="${endPageNum}" var="num">
					  ...
					  <c:if test="${select != num }">
					  	<a href="/CS/board/NoticeListPage?num=${num}">${num }</a>
					 </c:if>
			...
			<c:if test="${next}">
				 <span>[ <a href="/CS/board/NoticeListPage?num=${endPageNum + 1}">다음</a> ] </span>
			</c:if>
